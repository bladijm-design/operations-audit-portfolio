...
[![Project](https://img.shields.io/badge/View_Project-blue?style=flat-square)](/01-sentinel-automated-audit/)

```mermaid
graph LR
    A[50+ Vendor<br/>Data Feeds] --> B[SENTINEL<br/>Python Engine]
    B --> C{Control<br/>Test}
    C -->|PASS| D[Log & Monitor]
    C -->|FAIL| E[Finding Generated]
    E --> F[Severity Classification]
    F --> G[Management Action<br/>Assigned]
...
[![Project](https://img.shields.io/badge/View_Project-blue?style=flat-square)](/02-mnr-root-cause-engine/)

```mermaid
graph TD
    A[Daily Report<br/>800+ shipments] --> B[Triage Engine]
    B --> C[API Batch Query<br/>Tracking Events]
    C --> D[Cross-Reference<br/>7 Data Sources]
    D --> E[Root Cause<br/>Classification]
...
[![Project](https://img.shields.io/badge/View_Project-blue?style=flat-square)](/03-vendor-compliance-framework/)

```mermaid
graph LR
    A[Vendor<br/>Performance Data] --> B[Automated<br/>SLA Check]
    B --> C[Score<br/>Calculation]
    C --> D{Threshold<br/>Breach?}
    D -->|Yes| E[Escalation<br/>Protocol]
    D -->|No| F[Compliance<br/>Log]
    E --> G[Management<br/>Action]
    G --> H[Track to<br/>Resolution]
```
...
[![Project](https://img.shields.io/badge/View_Project-blue?style=flat-square)](/04-tcap-capacity-audit/)

```mermaid
graph TD
    A[Capacity<br/>Dashboard] --> B[Constraint<br/>Detection]
    B --> C[Utilization<br/>Validation]
    C --> D{Actionable?}
    D -->|Yes| E[Request<br/>Additional Capacity]
    D -->|No| F[Log as Noise]
    E --> G[Track<br/>Utilization KPI]
    G --> H[Monthly<br/>Performance Review]
    H --> I[Root Cause<br/>of Underperformance]
    I --> B
```
...
[![Project](https://img.shields.io/badge/View_Project-blue?style=flat-square)](/06-dummy-lanes-investigation/)
...
```mermaid
pie title Technical Proficiency Distribution
    "Python (Automation & Pipelines)" : 35
    "SQL (Analysis & Reporting)" : 25
    "Excel/Power BI (Dashboards)" : 20
    "API Integration (REST)" : 10
    "Selenium (Browser Automation)" : 10
```
...
```mermaid
graph TD
    A[1. SCOPE<br/>Define risk areas<br/>& objectives] --> B[2. ASSESS<br/>Identify controls<br/>& test criteria]
    B --> C[3. TEST<br/>Execute control tests<br/>using data analytics]
    C --> D[4. REPORT<br/>Document findings<br/>with severity & evidence]
    D --> E[5. ACTION<br/>Assign management<br/>actions with deadlines]
    E --> F[6. VALIDATE<br/>Verify completion<br/>against success criteria]
    F --> G[7. MONITOR<br/>Continuous oversight<br/>& trend tracking]
    G --> A
```
...
```mermaid
gantt
    title Professional Impact Timeline
    dateFormat YYYY-MM
    axisFormat %Y

    section Operations Analyst
    205 config audits (98% success)        :2022-07, 2023-12
    €50K savings via automation            :2023-01, 2023-12

    section Team Lead
    NCA process (7,776 trucks, €20M)       :2024-01, 2025-01
    Dummy lanes audit (€30K savings)       :2024-06, 2024-12
    TCAP audit launch (62→81% util)        :2025-07, 2026-04
    Team incident reduction (-75%)         :2024-01, 2025-08

    section Operations Manager
    SENTINEL deployed ($104K/week)         :2025-09, 2026-06
    MNR automation (95% time reduction)    :2026-04, 2026-06
    Vendor framework (1,700+ cases/yr)     :2025-09, 2026-06
```
